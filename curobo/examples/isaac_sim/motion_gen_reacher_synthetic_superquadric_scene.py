...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--skip_pipeline_stage_one",
        type=bool,
        default=False,
        help="Wheter to use a synthetic depth image or snythetic pointcloud",)
    parser.add_argument(
        "--checkpoint_folder",
        type=str,
...
from project_3dv.perception.superquadric import fits_to_curobo_obstacles
...
def _make_synthetic_depth_image(
    ...
    Image.fromarray(depth_u16).save("syn_depth.png")
    return depth_u16, K

def infer_superquadric_world() -> WorldConfig:

    fitter = SuperquadricFitter(n_restarts=2, n_lm_rounds=8, subsample=256)
    pipe    = TabletopPerception(
            ...
            depth_margin=0.30,
            xy_radius=0.80,
            split_min_points=9999,
        )

    pts = None
    if args.skip_pipeline_stage_one:
        pts = _make_synthetic_tabletop()
    else:
        depth, K = _make_synthetic_depth_image()
        pts = depth_to_pointcloud(depth, K, depth_scale=1000.0)

    result = pipe.run(pts)
    sq_multis = [
        fitter.fit_adaptive(seg.points, shape_hint=seg.shape_type)
        for seg in result.objects
    ]
    flat_fits = [sq for m in sq_multis for sq in m.primitives]
    obstacles = fits_to_curobo_obstacles(flat_fits)

    superquadrics = []
    for obs in obstacles:
        print(obs)
        if not obs['quality_ok']:
            continue
        rotation = Rotation.from_matrix(np.array(obs['params']['rotation']).reshape(3,3))

        superquadrics.append(
            Superquadric(
                name=f"sq_{obs['id']}",
                pose=obs['params']['translation'] + rotation.as_quat().tolist(),
                radii=obs['params']['scale'],
                eps=obs['params']['shape'],
                color=[255, 128, 0, 255],
            )
        )
    ...
    if not superquadrics:
        raise RuntimeError(
            f"SuperDec did not return any active primitives from synthetic tabletop depthimage."
