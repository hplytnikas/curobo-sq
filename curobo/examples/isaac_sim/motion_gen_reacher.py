...
def main():
    ...
    ### Addition of superquadrics #################################################
    import sys
    sys.path.append("/home/haroldas/3DV/superdec")
    from superdec_to_curobo_world import superdec_to_curobo_world
    
    sq_cfg = superdec_to_curobo_world(
        ply_path="/home/haroldas/3DV/superdec/examples/chair.ply",
        checkpoint_folder="/home/haroldas/3DV/superdec/checkpoints/normalized"
    )
    
    existing_meshes = world_cfg1.mesh if world_cfg1.mesh is not None else []
    sq_meshes = sq_cfg.mesh if sq_cfg.mesh is not None else []
    
    
    if len(sq_meshes) == 0:
        print("No meshes found in superquadric config.-------------------------")
    else:
        print(f"Successfully generated {len(sq_meshes)} meshes!")


    scale_factor = 1 
    
    for mesh in sq_meshes:
        # 1. Apply the scale multiplier (X, Y, Z)
        mesh.scale = [scale_factor, scale_factor, scale_factor]
        
        # 2. Shift it forward and up so it sits on the table
        mesh.pose[0] += -0.29955 
        mesh.pose[1] += -0.68389  
        mesh.pose[2] += 0.13559 
        mesh.pose[3] = 0.7071  # qw
        mesh.pose[4] = 0.7071  # qx
        mesh.pose[5] = 0.0  # qy
        mesh.pose[6] = 0.0  # qz
    # -------------------------------------------------
        
    combined_meshes = existing_meshes + sq_meshes
    world_cfg = WorldConfig(cuboid=world_cfg_table.cuboid, mesh=combined_meshes)
    ###############################################################################
