...
std::vector<torch::Tensor>
swept_sphere_superquadric_clpt(const torch::Tensor sphere_position,
                torch::Tensor distance,
                torch::Tensor closest_point,
                torch::Tensor sparsity_idx,
                const torch::Tensor weight,
...
std::vector<torch::Tensor>
sphere_superquadric_clpt_wrapper(const torch::Tensor sphere_position,
                                 torch::Tensor distance,
                                 torch::Tensor closest_point,
                                 torch::Tensor sparsity_idx,
                                 const torch::Tensor weight,
                                 ...
                                 const bool compute_esdf)
{
  ...
  CHECK_INPUT(weight);
  CHECK_INPUT(activation_distance);
  CHECK_INPUT(sq_params);
  CHECK_INPUT(sq_enable);
  CHECK_INPUT(n_env_sq);
  CHECK_INPUT(env_query_idx);

  return sphere_superquadric_clpt(
    sphere_position,
    distance,
    closest_point,
    sparsity_idx,
...
std::vector<torch::Tensor>
swept_sphere_superquadric_clpt_wrapper(const torch::Tensor sphere_position,
                                       torch::Tensor distance,
                                       torch::Tensor closest_point,
                                       torch::Tensor sparsity_idx,
                                       const torch::Tensor weight,
                                       ...
                                       const bool sum_collisions)
{
  ...
  return swept_sphere_superquadric_clpt(
    sphere_position,
    distance,
    closest_point,
    sparsity_idx,
...
void launch_superquadric_distance(
    torch::Tensor sphere_centers,
    torch::Tensor sphere_radii,
    torch::Tensor superquadrics,
    torch::Tensor distances)
{
    int n = sphere_centers.size(0);

    auto stream = at::cuda::getDefaultCUDAStream();
    launch_sphere_sq_distance_kernel(
      reinterpret_cast<float3*>(sphere_centers.data_ptr()),
      sphere_radii.data_ptr<float>(),
      reinterpret_cast<Superquadric*>(superquadrics.data_ptr()),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
  ...
  m.def("closest_point_superquadric",    &sphere_superquadric_clpt_wrapper,
      "Closest Point Superquadric(curobolib)");
    m.def("swept_closest_point_superquadric", &swept_sphere_superquadric_clpt_wrapper,
      "Swept Closest Point Superquadric(curobolib)");
  m.def("superquadric_distance", &launch_superquadric_distance);
