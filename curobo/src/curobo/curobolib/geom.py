...
def superquadric_distance(
    sphere_centers,
    sphere_radii,
    superquadrics
):
    n = sphere_centers.shape[0]
...
class SdfSphereSuperquadric(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        query_sphere,
        ...
        # The CUDA kernel expects [batch, horizon, n_spheres] and [batch, horizon, n_spheres, 4].
        expected_dist_shape = (batch_size, horizon, n_spheres)
        expected_grad_shape = (batch_size, horizon, n_spheres, 4)
        if tuple(out_buffer.shape) != expected_dist_shape:
            out_buffer = out_buffer.new_zeros(expected_dist_shape)
        if tuple(grad_out_buffer.shape) != expected_grad_shape:
            grad_out_buffer = grad_out_buffer.new_zeros(expected_grad_shape)
        if tuple(sparsity_idx.shape) != expected_dist_shape:
            sparsity_idx = torch.zeros(
                expected_dist_shape,
                device=query_sphere.device,
                dtype=torch.uint8,
            )
        ...
        try:
            ...
            debug_ctx = (
                "closest_point_superquadric failed with shapes: "
                + f"query={tuple(query_sphere.shape)}, "
                + f"out={tuple(out_buffer.shape)}, "
                + f"grad={tuple(grad_out_buffer.shape)}, "
                + f"sparse={tuple(sparsity_idx.shape)}, "
                + f"sq_params={tuple(sq_params.shape)}, "
                + f"sq_enable={tuple(sq_enable.shape)}, "
                + f"n_env_sq={tuple(n_env_sq.shape)}, "
                + f"env_query_idx={tuple(env_query_idx.shape) if env_query_idx is not None else None}, "
                + f"max_nobs={max_nobs}, batch={batch_size}, horizon={horizon}, n_spheres={n_spheres}, "
                + f"use_batch_env={use_batch_env}, sum_collisions={sum_collisions}, compute_esdf={compute_esdf}"
            )
...
class SdfSweptSphereSuperquadric(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        query_sphere,
        ...
        if not hasattr(geom_cu, "swept_closest_point_superquadric"):
            raise RuntimeError(
                "geom_cu was built without swept superquadric support. Rebuild extension "
                "to load swept_closest_point_superquadric."
            )

        # Match expected output buffer shapes to avoid stale planner-state mismatches.
        expected_dist_shape = (batch_size, horizon, n_spheres)
        expected_grad_shape = (batch_size, horizon, n_spheres, 4)
        if tuple(out_buffer.shape) != expected_dist_shape:
            out_buffer = out_buffer.new_zeros(expected_dist_shape)
        ...
        r = geom_cu.swept_closest_point_superquadric(
            query_sphere,
            out_buffer,
            grad_out_buffer,
            sparsity_idx,
