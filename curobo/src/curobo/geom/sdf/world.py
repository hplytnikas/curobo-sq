...
# CuRobo
from curobo.curobolib.geom import (
    SdfSphereOBB,
    SdfSphereSuperquadric,
    SdfSweptSphereOBB,
    SdfSweptSphereSuperquadric,
)
...
class CollisionQueryBuffer:
    ...
    #: Buffer to store signed distance cost value for Superquadric world obstacles.
    superquadric_collision_buffer: Optional[CollisionBuffer] = None
    ...
    def __post_init__(self):
        ...
        if self.shape is None:
            ...
            elif self.superquadric_collision_buffer is not None:
                self.shape = self.superquadric_collision_buffer.shape
    ...
    def __mul__(self, scalar: float) -> CollisionQueryBuffer:
        ...
        if self.superquadric_collision_buffer is not None:
            self.superquadric_collision_buffer = self.superquadric_collision_buffer * scalar
    ...
    def clone(self) -> CollisionQueryBuffer:
        ...
        prim_buffer = superquadric_buffer = mesh_buffer = blox_buffer = voxel_buffer = None
        if self.primitive_collision_buffer is not None:
            prim_buffer = self.primitive_collision_buffer.clone()
        if self.superquadric_collision_buffer is not None:
            superquadric_buffer = self.superquadric_collision_buffer.clone()
        if self.mesh_collision_buffer is not None:
        ...
        return CollisionQueryBuffer(
            primitive_collision_buffer=prim_buffer,
            superquadric_collision_buffer=superquadric_buffer,
            mesh_collision_buffer=mesh_buffer,
            blox_collision_buffer=blox_buffer,
    ...
    def initialize_from_shape(
        ...
        """
        primitive_buffer = superquadric_buffer = mesh_buffer = blox_buffer = voxel_buffer = None
        ...
        if "superquadric" in collision_types and collision_types["superquadric"]:
            superquadric_buffer = CollisionBuffer.initialize_from_shape(shape, tensor_args)
        ...
        return CollisionQueryBuffer(
            primitive_collision_buffer=primitive_buffer,
            superquadric_collision_buffer=superquadric_buffer,
            mesh_collision_buffer=mesh_buffer,
            blox_collision_buffer=blox_buffer,
            voxel_collision_buffer=voxel_buffer,
    ...
    def create_from_shape(
        ...
        if "superquadric" in collision_types and collision_types["superquadric"]:
            self.superquadric_collision_buffer = CollisionBuffer.initialize_from_shape(
                shape, tensor_args
            )
        if "mesh" in collision_types and collision_types["mesh"]:
    ...
    def update_buffer_shape(
        ...
        if self.shape is None:  # buffers not initialized:
            ...
            if self.superquadric_collision_buffer is not None:
                self.superquadric_collision_buffer.update_buffer_shape(shape, tensor_args)
            if self.mesh_collision_buffer is not None:
    ...
    def get_gradient_buffer(
        ...
        if self.superquadric_collision_buffer is not None:
            superquadric_buffer = self.superquadric_collision_buffer.grad_distance_buffer
            if current_buffer is None:
                current_buffer = superquadric_buffer.clone()
            else:
                current_buffer += superquadric_buffer
...
class WorldPrimitiveCollision(WorldCollision):
    ...
    def __init__(self, config: WorldCollisionConfig):
        ...
        self._sq_tensor_list = None
        self._env_n_superquadrics = None
        self._env_superquadrics_names = None
        self._init_cache()
    ...
    def _init_cache(self):
        ...
        if (
            ...
            and "superquadric" in self.cache
            and self.cache["superquadric"] not in [None, 0]
        ):
            self._create_superquadric_cache(self.cache["superquadric"])
    ...
    def get_obstacle_names(self, env_idx: int = 0) -> List[str]:
        ...
        obb_names = [] if self._env_obbs_names is None else self._env_obbs_names[env_idx]
        sq_names = (
            [] if self._env_superquadrics_names is None else self._env_superquadrics_names[env_idx]
        )
        return obb_names + sq_names + base_obstacles
    ...
    def load_batch_collision_model(self, world_config_list: List[WorldConfig]):
        ...
        if max_obb < 1:
            ...
            if self._env_n_obbs is not None:
                self._env_n_obbs[:] = 0
        else:
    ...
    def _load_collision_model_in_cache(
        ...
        self._load_superquadrics_in_cache(
            world_config, env_idx=env_idx, fix_cache_reference=fix_cache_reference
        )
    ...
    def _create_superquadric_cache(self, superquadric_cache: int):
        """Create cache for superquadric obstacles."""
        sq_params = torch.zeros(
            (self.n_envs, superquadric_cache, 12),
            dtype=self.tensor_args.dtype,
            device=self.tensor_args.device,
        )
        sq_params[..., :3] = 0.01
        sq_params[..., 3:5] = 1.0
        sq_params[..., 11] = 1.0
        sq_enable = torch.zeros(
            (self.n_envs, superquadric_cache),
            dtype=torch.uint8,
            device=self.tensor_args.device,
        )
        self._sq_tensor_list = [sq_params, sq_enable]
        self._env_n_superquadrics = torch.zeros(
            (self.n_envs), device=self.tensor_args.device, dtype=torch.int32
        )
        self._env_superquadrics_names = [
            [None for _ in range(superquadric_cache)] for _ in range(self.n_envs)
        ]

    def _superquadric_to_tensor(self, sq: Superquadric) -> torch.Tensor:
        pose = sq.pose
        if pose is None:
    ...
    def _load_superquadrics_in_cache(
        self, world_config: WorldConfig, env_idx: int = 0, fix_cache_reference: bool = False
    ):
        sq_objs = world_config.superquadric
        max_sq = len(sq_objs)

        if max_sq < 1:
            if self._sq_tensor_list is not None:
                self._sq_tensor_list[1][env_idx, :] = 0
                self._env_n_superquadrics[env_idx] = 0
            return

        if self._sq_tensor_list is None or self._sq_tensor_list[0].shape[1] < max_sq:
            if not fix_cache_reference:
                log_info("Creating Superquadric cache" + str(max_sq))
                self._create_superquadric_cache(max_sq)
            else:
                log_error("number of superquadrics is larger than collision cache")

        params = torch.stack([self._superquadric_to_tensor(x) for x in sq_objs], dim=0)
        names = [x.name for x in sq_objs]

        self._sq_tensor_list[0][env_idx, :max_sq, :] = params
        self._sq_tensor_list[1][env_idx, :max_sq] = 1
        self._sq_tensor_list[1][env_idx, max_sq:] = 0
        self._env_n_superquadrics[env_idx] = max_sq
        self._env_superquadrics_names[env_idx][:max_sq] = names
        self.collision_types["superquadric"] = True

    def _load_batch_superquadrics(self, world_config_list: List[WorldConfig]):
        sq_len = []
        for world in world_config_list:
            sq_len.append(len(world.superquadric) if world.superquadric is not None else 0)

        max_sq = max(sq_len) if len(sq_len) > 0 else 0
        if max_sq < 1:
            if self._sq_tensor_list is not None:
                self._sq_tensor_list[1][:] = 0
            if self._env_n_superquadrics is not None:
                self._env_n_superquadrics[:] = 0
            return

        reset_buffers = False
        if self._env_n_superquadrics is not None and len(world_config_list) != len(
            self._env_n_superquadrics
        ):
            reset_buffers = True
            self.n_envs = len(world_config_list)
            self._env_n_superquadrics = torch.zeros(
        ...
        if self._sq_tensor_list is not None and self._sq_tensor_list[0].shape[1] < max_sq:
            reset_buffers = True

        if self._sq_tensor_list is None or reset_buffers:
            self._create_superquadric_cache(max_sq)

        for env_idx, world in enumerate(world_config_list):
            n_sq = sq_len[env_idx]
            if n_sq > 0:
                params = torch.stack(
                    [self._superquadric_to_tensor(x) for x in world.superquadric], dim=0
                )
                names = [x.name for x in world.superquadric]
                self._sq_tensor_list[0][env_idx, :n_sq, :] = params
                self._sq_tensor_list[1][env_idx, :n_sq] = 1
                self._env_superquadrics_names[env_idx][:n_sq] = names
            self._sq_tensor_list[1][env_idx, n_sq:] = 0
            self._env_n_superquadrics[env_idx] = n_sq
    ...
    def enable_obstacle(
        ...
        if (
            self._env_superquadrics_names is not None
            and name in self._env_superquadrics_names[env_idx]
        ):
            return self.enable_superquadric(enable, name, None, env_idx)
        log_warn("obstacle not found in primitive world model: " + name)
        return None
    ...
    def enable_superquadric(
        ...
        """Enable/Disable superquadric in collision checking functions."""
        if env_obj_idx is not None:
            self._sq_tensor_list[1][env_obj_idx] = int(enable)
        else:
            obs_idx = self.get_superquadric_idx(name, env_idx)
            self._sq_tensor_list[1][env_idx, obs_idx] = int(enable)

    def update_superquadric_pose(
        self,
        w_obj_pose: Optional[Pose] = None,
        obj_w_pose: Optional[Pose] = None,
        name: Optional[str] = None,
        ...
        """Update pose of an existing superquadric obstacle."""
        obj_w_pose = self._get_obstacle_poses(w_obj_pose, obj_w_pose)
        pose_vec = obj_w_pose.inverse().get_pose_vector().view(-1)
        # Store superquadric pose in world frame as [x, y, z, qx, qy, qz, qw].
        sq_pose = torch.stack(
            [
                pose_vec[0],
                pose_vec[1],
        ...
        if env_obj_idx is not None:
            self._sq_tensor_list[0][env_obj_idx, 5:12] = sq_pose
        else:
            obs_idx = self.get_superquadric_idx(name, env_idx)
            self._sq_tensor_list[0][env_idx, obs_idx, 5:12] = sq_pose
    ...
    def get_superquadric_idx(
        self,
        name: str,
        env_idx: int = 0,
    ) -> int:
        """Get index of a superquadric obstacle in the world."""
        if name not in self._env_superquadrics_names[env_idx]:
            log_error("Obstacle with name: " + name + " not found in current world", exc_info=True)
        return self._env_superquadrics_names[env_idx].index(name)
    ...
    def get_sphere_distance(
        ...
        """
        has_primitive = "primitive" in self.collision_types and self.collision_types["primitive"]
        has_superquadric = (
            "superquadric" in self.collision_types and self.collision_types["superquadric"]
        )
        if not has_primitive and not has_superquadric:
        ...
        if has_primitive:
            use_batch_env = True
            env_query_idx_prim = env_query_idx
            if env_query_idx_prim is None:
                use_batch_env = False
                env_query_idx_prim = self._env_n_obbs
        ...
        if has_superquadric:
            use_batch_env_sq = True
            env_query_idx_sq = env_query_idx
            if env_query_idx_sq is None:
                use_batch_env_sq = False
                env_query_idx_sq = self._env_n_superquadrics

            dist_terms.append(
                SdfSphereSuperquadric.apply(
                    query_sphere,
                    collision_query_buffer.superquadric_collision_buffer.distance_buffer,
                    collision_query_buffer.superquadric_collision_buffer.grad_distance_buffer,
                    collision_query_buffer.superquadric_collision_buffer.sparsity_index_buffer,
                    weight,
                    activation_distance,
                    self._sq_tensor_list[0],
                    self._sq_tensor_list[1],
                    self._env_n_superquadrics,
                    env_query_idx_sq,
                    self._sq_tensor_list[0].shape[1],
                    b,
                    h,
                    n,
                    True,
        ...
        if compute_esdf:
            d_val = dist_terms[0]
            for d in dist_terms[1:]:
                d_val = torch.maximum(d_val, d)
            return d_val
    ...
    def get_swept_sphere_distance(
        ...
        if has_primitive:
            use_batch_env_prim = True
            env_query_idx_prim = env_query_idx
            if env_query_idx_prim is None:
                use_batch_env_prim = False
                env_query_idx_prim = self._env_n_obbs
        ...
        if has_superquadric:
            ...
            dist_terms.append(
                SdfSweptSphereSuperquadric.apply(
                    query_sphere,
                    collision_query_buffer.superquadric_collision_buffer.distance_buffer,
                    collision_query_buffer.superquadric_collision_buffer.grad_distance_buffer,
                    collision_query_buffer.superquadric_collision_buffer.sparsity_index_buffer,
