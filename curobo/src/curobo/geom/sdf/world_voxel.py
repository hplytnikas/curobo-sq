...
class WorldVoxelCollision(WorldMeshCollision):
    ...
    def get_sphere_distance(
        ...
        if (
            ("primitive" not in self.collision_types or not self.collision_types["primitive"])
            and (
                "superquadric" not in self.collision_types
                or not self.collision_types["superquadric"]
            )
            and ("mesh" not in self.collision_types or not self.collision_types["mesh"])
        ):
            return dist
