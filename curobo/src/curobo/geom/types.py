...
class Superquadric(Obstacle):
    ...
    #: Principal radii of the superquadric [a1, a2, a3].
    radii: List[float] = field(default_factory=lambda: [0.05, 0.05, 0.05])

    #: Shape exponents [e1, e2].
    eps: List[float] = field(default_factory=lambda: [1.0, 1.0])

    def __post_init__(self):
        """Post initialization checks if pose and parameters were set."""
        if self.pose is None:
            log_error("Superquadric Obstacle requires Pose")
        if len(self.radii) != 3:
            log_error("Superquadric.radii must have length 3")
        if len(self.eps) != 2:
            log_error("Superquadric.eps must have length 2")

    def get_trimesh_mesh(self, process: bool = True, process_color: bool = True) -> trimesh.Trimesh:
        """Create a trimesh instance from the superquadric representation.

        This mesh is intended for visualization and generic conversions; collision checks use the
        native superquadric representation.
        """
        m = trimesh.creation.icosphere(radius=1.0)
        m.apply_scale(self.radii)
        if self.color is not None:
            color_visual = trimesh.visual.color.ColorVisuals(
                mesh=m, face_colors=self.color, vertex_colors=self.color
            )
...
class WorldConfig(Sequence):
    ...
    def create_obb_world(current_world: WorldConfig) -> WorldConfig:
        ...
        if current_world.superquadric is not None and len(current_world.superquadric) > 0:
            superquadric_obb = [x.get_cuboid() for x in current_world.superquadric]
    ...
    def create_mesh_world(current_world: WorldConfig, process: bool = False) -> WorldConfig:
        ...
        if current_world.superquadric is not None and len(current_world.superquadric) > 0:
            superquadric_mesh = [x.get_mesh(process=process) for x in current_world.superquadric]
        if current_world.voxel is not None and len(current_world.voxel) > 0:
    ...
    def create_collision_support_world(
        ...
        return WorldConfig(
            ...
            cuboid=cuboid_obb,
            superquadric=current_world.superquadric,
    ...
    def get_cache_dict(self) -> Dict[str, int]:
        ...
        cache = {
            "obb": len(self.cuboid),
            "mesh": len(self.mesh),
            "superquadric": len(self.superquadric),
        }
        return cache
