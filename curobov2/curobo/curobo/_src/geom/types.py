...
class Superquadric(Obstacle):
    """Obstacle represented as a superquadric primitive.

    The superquadric surface is defined by:
        F(x,y,z) = ((|x/sx|^{2/ε₂} + |y/sy|^{2/ε₂})^{ε₂/ε₁} + |z/sz|^{2/ε₁}) = 1

    Special cases: ε₁=ε₂=1 → ellipsoid, ε₁=ε₂→0 → box, ε₁=1,ε₂→0 → cylinder.
    """

    #: Semi-axes [sx, sy, sz] in meters (must be positive).
    radii: List[float] = field(default_factory=lambda: [0.1, 0.1, 0.1])

    #: Shape exponents [eps1, eps2] — typically in (0, 2].
    #: eps1 controls north-south shape, eps2 controls east-west cross-section.
    shape: List[float] = field(default_factory=lambda: [1.0, 1.0])

    def __post_init__(self):
        """Validate that pose and radii are provided."""
        if self.pose is None:
            log_and_raise("Superquadric requires a pose [x, y, z, qw, qx, qy, qz]")
        if len(self.radii) != 3:
            log_and_raise("Superquadric.radii must have 3 elements [sx, sy, sz]")
        if len(self.shape) != 2:
            log_and_raise("Superquadric.shape must have 2 elements [eps1, eps2]")

    def get_trimesh_mesh(
        self, process: bool = True, process_color: bool = True, transform_with_pose: bool = False
    ) -> "trimesh.Trimesh":
        """Approximate the superquadric as a mesh for visualization."""
        import trimesh
        n = 32
        ...
        def sq_cos(angle, exp):
            c = np.cos(angle)
            return np.sign(c) * np.abs(c) ** exp

        def sq_sin(angle, exp):
            s = np.sin(angle)
            return np.sign(s) * np.abs(s) ** exp

        U, V = np.meshgrid(u, v)
        X = sx * sq_cos(U, e1) * sq_cos(V, e2)
        Y = sy * sq_cos(U, e1) * sq_sin(V, e2)
        Z = sz * sq_sin(U, e1)
        ...
        if self.color is not None:
            m.visual.vertex_colors = self.color
        if transform_with_pose:
            m.apply_transform(self.get_transform_matrix())
        return m
...
class SceneCfg(Sequence):
    ...
    #: List of superquadric obstacles.
    superquadric: Optional[List[Superquadric]] = None
    ...
    def clone(self) -> SceneCfg:
        ...
        return SceneCfg(
            ...
            superquadric=self.superquadric.copy() if self.superquadric is not None else None,
        )
    ...
    def create(data_dict: Dict[str, Any]) -> SceneCfg:
        ...
        if "superquadric" in data_dict.keys():
            superquadric = [
                Superquadric(name=x, **data_dict["superquadric"][x])
                for x in data_dict["superquadric"]
            ]
    ...
    def create_mesh_scene(current_world: SceneCfg, process: bool = False) -> SceneCfg:
        ...
        sq_obb = []
        if current_world.superquadric is not None and len(current_world.superquadric) > 0:
            sq_obb = [x.get_mesh(process=process) for x in current_world.superquadric]
    ...
    def add_obstacle(self, obstacle: Obstacle):
        ...
        if isinstance(obstacle, Mesh):
            ...
            self.superquadric.append(obstacle)
        else:
