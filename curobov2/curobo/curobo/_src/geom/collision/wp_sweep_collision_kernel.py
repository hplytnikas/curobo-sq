...
def _raw(f):
    """Return the underlying Python callable from a wp.Function, or f itself."""
    return f.func if isinstance(f, wp.Function) and f.func is not None else f


for _module_path in OBSTACLE_SDF_MODULES:
    _data_module = import_module(_module_path)
    _obs_fn = _raw(getattr(_data_module, "is_obs_enabled"))
    _transform_fn = _raw(getattr(_data_module, "load_obstacle_transform"))
    _sdf_only_fn = _raw(getattr(_data_module, "compute_local_sdf"))
    _sdf_fn = _raw(getattr(_data_module, "compute_local_sdf_with_grad"))

    # Register with wp.func - adds overloads on subsequent iterations.
    # _raw() unwraps wp.Function objects (from @wp.func in source modules)
    # so wp.func() always receives a plain Python callable.
