...
from curobo._src.geom.types import Cuboid, Mesh, Obstacle, SceneCfg, Superquadric, VoxelGrid
...
class SceneData:
    ...
    #: Superquadric tensor storage. None if no superquadrics configured.
    superquadrics: Optional[SuperquadricData] = None
    ...
    # Factory Methods
    # -------------------------------------------------------------------------
    def get_valid_data(self) -> List[Union[CuboidData, MeshData, VoxelData, SuperquadricData]]:
        valid_data = []
        ...
        if self.superquadrics is not None:
            valid_data.append(self.superquadrics)
        return valid_data
    ...
    def create_cache(
        ...
        superquadric_cache: Optional[int] = None,
    ) -> SceneData:
        ...
        if superquadric_cache is not None and superquadric_cache > 0:
            superquadrics = SuperquadricData.create_cache(superquadric_cache, num_envs, device_cfg)
    ...
    def from_scene_cfg(
        ...
        # Handle superquadrics
        has_sqs = (
            hasattr(scene_cfg, "superquadric")
            and scene_cfg.superquadric
            and len(scene_cfg.superquadric) > 0
        )
        if has_sqs:
            max_sqs = superquadric_cache or len(scene_cfg.superquadric)
            superquadrics = SuperquadricData.from_scene_cfg(
                scene_cfg, device_cfg, env_idx, num_envs, max_sqs
            )
        elif superquadric_cache is not None and superquadric_cache > 0:
            superquadrics = SuperquadricData.create_cache(superquadric_cache, num_envs, device_cfg)
    ...
    def from_batch_scene_cfg(
        ...
        # Check if any scene has superquadrics
        sq_counts = [
            len(cfg.superquadric) if (hasattr(cfg, "superquadric") and cfg.superquadric) else 0
            for cfg in scene_cfg_list
        ]
        if max(sq_counts) > 0 or (superquadric_cache is not None and superquadric_cache > 0):
            superquadrics = SuperquadricData.from_batch_scene_cfg(
                scene_cfg_list, device_cfg, superquadric_cache
            )
    ...
    def add_obstacle(self, obstacle: Obstacle, env_idx: int = 0) -> int:
        ...
        if isinstance(obstacle, Cuboid):
            ...
            return 0
        elif isinstance(obstacle, Superquadric):
            if self.superquadrics is None:
                log_and_raise("Superquadric cache not initialized")
            return self.superquadrics.add(obstacle, env_idx)
        else:
    ...
    def update_obstacle_pose(
        ...
        if self.superquadrics is not None:
            try:
                self.superquadrics.update_pose(name, w_obj_pose=pose, env_idx=env_idx)
                return
            except (RuntimeError, ValueError):
                pass
    ...
    def enable_obstacle(
        ...
        if self.superquadrics is not None:
            try:
                self.superquadrics.set_enabled(name, enabled, env_idx)
                return
            except (RuntimeError, ValueError):
                pass
    ...
    def get_obstacle_names(self, env_idx: int = 0) -> List[str]:
        ...
        if self.superquadrics is not None:
            names.extend(self.superquadrics.get_names(env_idx))
        return names
    ...
    def clear(self, env_idx: Optional[int] = None) -> None:
        ...
        if self.superquadrics is not None:
            self.superquadrics.clear(env_idx)
    ...
    def load_from_scene_cfg(
        ...
        if hasattr(scene_cfg, "superquadric") and scene_cfg.superquadric:
            for sq in scene_cfg.superquadric:
                self.add_obstacle(sq, env_idx)
    ...
    def has_superquadrics(self) -> bool:
        """Check if superquadric data is available."""
        return self.superquadrics is not None
    ...
    def get_active_types(self) -> dict:
        ...
        return {
            ...
            "superquadric": self.has_superquadrics(),
        }
