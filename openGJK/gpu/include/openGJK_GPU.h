...
/**
 * @brief  Fast approximation of pow(a,b) using exp2 and log2. Only for positive a.
 */
__device__ inline float fast_pow(float a, float b)
{
    if (a <= 0.0f) {
        return 0.0f;
    }
...
/**
 * @brief Cross function implementation.
 */
...
/**
 * @brief Support function for Minkowski difference of superquadrics.
 */
__device__ float3 support_minkowski_superquadric(
        float3 dir,
...
/**
 * @brief Support function for a sphere.
 */
__device__ inline float3 support_sphere(float3 dir,
                                         float radius,
                                         float3 center)
{
    ...
    float3 p;
    p.x = center.x + radius * dir.x * inv;
    p.y = center.y + radius * dir.y * inv;
    p.z = center.z + radius * dir.z * inv;

    return p;
}

/**
 * @brief Support function for Minkowski difference of superquadric and sphere.
 */
...
/**
 * @brief Compute signed distance between a superquadric and a sphere.
 *
 * Positive value means separation, zero means touching, and negative value
 * means penetration depth.
 *
 * @param sq            Superquadric description (center, orientation, radii, exponents)
 * @param sphere_radius Sphere radius (must be non-negative)
 * @param sphere_center Sphere center in world coordinates
 * @return Signed distance in world units
 */
...
/**
 * @brief Compute signed distance for a batch of superquadric-sphere queries.
 *
 * Each query has its own superquadric parameters and sphere [x,y,z,r].
 * The launch is asynchronous on the provided CUDA stream.
 *
 * @param sq_params         Pointer to [n,12] params
 *                          [a1,a2,a3,e1,e2,px,py,pz,qx,qy,qz,qw].
 * @param sphere_xyzr       Pointer to [n,4] spheres [x,y,z,r].
 * @param n                 Number of queries.
 * @param signed_distances  Output pointer to [n] signed distances.
 * @param stream            CUDA stream to enqueue work on.
 */
...
/**
 * @brief Compute signed distance for many spheres against one superquadric.
 *
 * The launch is asynchronous on the provided CUDA stream.
 *
 * @param sq_param          Pointer to [12] params
 *                          [a1,a2,a3,e1,e2,px,py,pz,qx,qy,qz,qw].
 * @param sphere_xyzr       Pointer to [n,4] spheres [x,y,z,r].
 * @param n                 Number of sphere queries.
 * @param signed_distances  Output pointer to [n] signed distances.
 * @param stream            CUDA stream to enqueue work on.
 */
...
/**
 * @brief Support function for superquadrics using fast_pow approximation. Returns the support point in the given direction.
 */
__device__ float3 support_superquadric_fast(float3 dir, const Superquadric& sq);

/**
 * @brief Rotate a vector by a quaternion. Used for orienting superquadrics.
 */
__device__ inline float3 quat_rotate(const float4& q, const float3& v)
{
...
__device__ inline float4 quat_conjugate(const float4& q)
{
