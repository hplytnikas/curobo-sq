...
// ============================================================================
// SUPERQUADRICS: Extension for superquadrics
// ============================================================================
__device__
float3 support_superquadric_fast(float3 dir,
                                 const Superquadric& sq)
{
  const float4 inv_quat = quat_conjugate(sq.quat);
  dir = quat_rotate(inv_quat, dir);

    // normalize direction
    float inv_len = rsqrtf(dir.x*dir.x + dir.y*dir.y + dir.z*dir.z + 1e-9f);
    ...
    float px = fast_pow(ax, sq.eps.y);
    float py = fast_pow(ay, sq.eps.y);
    float pz = fast_pow(az, sq.eps.x);

    // restore sign
    px = copysignf(px, dir.x);
    py = copysignf(py, dir.y);
    pz = copysignf(pz, dir.z);
    ...
    // project to surface
    float t1 = fast_pow(fabsf(p.x / sq.radii.x), 2.f / sq.eps.y);
    float t2 = fast_pow(fabsf(p.y / sq.radii.y), 2.f / sq.eps.y);

    float xy = fast_pow(t1 + t2, sq.eps.y / sq.eps.x);

    float z = fast_pow(fabsf(p.z / sq.radii.z), 2.f / sq.eps.x);

    float f = xy + z;

    float scale = rsqrtf(f);

    p.x *= scale;
    p.y *= scale;
    p.z *= scale;

    p = quat_rotate(sq.quat, p);
...
__device__
float3 support_minkowski_superquadric(
        float3 dir,
        const Superquadric& sqA,
        const Superquadric& sqB)
{
    float3 pA = support_superquadric_fast(dir, sqA);
    ...
    float3 pB = support_superquadric_fast(ndir, sqB);
...
__device__ float3 support_superquadric_sphere(float3 dir,
                   const Superquadric& sq,
                   float radius,
                   float3 sphere_center)
{
    float3 pA = support_superquadric_fast(dir, sq);

    float inv = rsqrtf(dir.x*dir.x + dir.y*dir.y + dir.z*dir.z + 1e-9f);

    float3 r;

    r.x = pA.x - sphere_center.x + radius * dir.x * inv;
    r.y = pA.y - sphere_center.y + radius * dir.y * inv;
    r.z = pA.z - sphere_center.z + radius * dir.z * inv;

    return r;
}

#define SQ_SPHERE_GJK_MAX_ITERS 64
#define SQ_SPHERE_EPA_MAX_ITERS 64
#define SQ_SPHERE_EPA_MAX_FACES 128
#define SQ_SPHERE_EPA_MAX_VERTICES (SQ_SPHERE_EPA_MAX_FACES + 4)
...
typedef struct {
  float3 vertices[SQ_SPHERE_EPA_MAX_VERTICES];
  SqSphereEPAFace faces[SQ_SPHERE_EPA_MAX_FACES];
  int num_vertices;
  int max_face_index;
} SqSphereEPAPolytope;

__device__ inline static float sq_dot(const float3& a, const float3& b) {
  return a.x * b.x + a.y * b.y + a.z * b.z;
}

__device__ inline static float3 sq_sub(const float3& a, const float3& b) {
  return make_float3(a.x - b.x, a.y - b.y, a.z - b.z);
}

__device__ inline static float3 sq_scale(const float3& v, float s) {
  return make_float3(v.x * s, v.y * s, v.z * s);
}

__device__ inline static float sq_len2(const float3& v) {
  return sq_dot(v, v);
}

__device__ inline static float sq_len(const float3& v) {
  return sqrtf(sq_len2(v));
}

__device__ inline static float3 sq_cross(const float3& a, const float3& b) {
  return make_float3(
      a.y * b.z - a.z * b.y,
      a.z * b.x - a.x * b.z,
      a.x * b.y - a.y * b.x);
...
__device__ inline static float3 sq_any_perpendicular(const float3& v) {
  if (fabsf(v.x) < fabsf(v.y)) {
    return sq_cross(v, make_float3(1.0f, 0.0f, 0.0f));
  }
  return sq_cross(v, make_float3(0.0f, 1.0f, 0.0f));
}

__device__ inline static void sq_simplex_push_front(
    SqSphereSimplex* simplex,
    const float3& p) {
  if (simplex->count < 4) {
    simplex->count++;
  }
...
__device__ inline static bool sq_handle_line(
    SqSphereSimplex* simplex,
    float3* direction) {
  const float3 a = simplex->points[0];
  const float3 b = simplex->points[1];
  const float3 ao = sq_scale(a, -1.0f);
  const float3 bo = sq_scale(b, -1.0f);
  const float3 ab = sq_sub(b, a);
  const float3 ba = sq_sub(a, b);

  if (sq_dot(ab, ao) > 0.0f) {
    const float3 ab_x_ao = sq_cross(ab, ao);
    float3 next_dir = sq_cross(ab_x_ao, ab);

    // Collinear case: if the origin lies on segment AB we are colliding.
    if (sq_len2(next_dir) < 1e-14f &&
        sq_dot(ab, ao) > 0.0f &&
        sq_dot(ba, bo) > 0.0f) {
      return true;
    }
    ...
    if (sq_len2(next_dir) < 1e-14f) {
      next_dir = sq_any_perpendicular(ab);
    }
    *direction = next_dir;
    return false;
  }
  ...
  simplex->points[0] = a;
  *direction = ao;
  return sq_len2(*direction) < 1e-14f;
}

__device__ inline static bool sq_handle_triangle(
    SqSphereSimplex* simplex,
    float3* direction) {
  const float3 a = simplex->points[0];
  const float3 b = simplex->points[1];
  ...
  const float3 ao = sq_scale(a, -1.0f);
  const float3 ab = sq_sub(b, a);
  const float3 ac = sq_sub(c, a);
  const float3 abc = sq_cross(ab, ac);

  const float3 ab_perp = sq_cross(ab, abc);
  if (sq_dot(ab_perp, ao) > 0.0f) {
    simplex->count = 2;
    simplex->points[0] = a;
    simplex->points[1] = b;
    return sq_handle_line(simplex, direction);
  }

  const float3 ac_perp = sq_cross(abc, ac);
  if (sq_dot(ac_perp, ao) > 0.0f) {
    simplex->count = 2;
    simplex->points[0] = a;
    simplex->points[1] = c;
    return sq_handle_line(simplex, direction);
  ...
  simplex->points[1] = c;
  simplex->points[2] = b;
  *direction = sq_scale(abc, -1.0f);
  return false;
}

__device__ inline static bool sq_handle_tetrahedron(
    SqSphereSimplex* simplex,
    float3* direction) {
  const float3 a = simplex->points[0];
  const float3 b = simplex->points[1];
  ...
  float3 abc = sq_cross(sq_sub(b, a), sq_sub(c, a));
  if (sq_dot(abc, sq_sub(d, a)) > 0.0f) {
    abc = sq_scale(abc, -1.0f);
  }
  if (sq_dot(abc, ao) > 0.0f) {
    simplex->count = 3;
    simplex->points[0] = a;
    simplex->points[1] = b;
    simplex->points[2] = c;
    return sq_handle_triangle(simplex, direction);
  }

  float3 acd = sq_cross(sq_sub(c, a), sq_sub(d, a));
  if (sq_dot(acd, sq_sub(b, a)) > 0.0f) {
    acd = sq_scale(acd, -1.0f);
  }
  if (sq_dot(acd, ao) > 0.0f) {
    simplex->count = 3;
    simplex->points[0] = a;
    simplex->points[1] = c;
    simplex->points[2] = d;
  ...
  float3 adb = sq_cross(sq_sub(d, a), sq_sub(b, a));
  if (sq_dot(adb, sq_sub(c, a)) > 0.0f) {
    adb = sq_scale(adb, -1.0f);
  }
  if (sq_dot(adb, ao) > 0.0f) {
    simplex->count = 3;
    simplex->points[0] = a;
    simplex->points[1] = d;
    simplex->points[2] = b;
...
__device__ inline static bool sq_update_simplex_and_direction(
    SqSphereSimplex* simplex,
    float3* direction) {
  switch (simplex->count) {
  case 1:
    *direction = sq_scale(simplex->points[0], -1.0f);
    return sq_len2(*direction) < 1e-14f;
  case 2:
    return sq_handle_line(simplex, direction);
  case 3:
    return sq_handle_triangle(simplex, direction);
  case 4:
    return sq_handle_tetrahedron(simplex, direction);
  default:
    break;
  }
  return false;
...
__device__ inline static bool run_gjk_superquadric_sphere(
    const Superquadric& sq,
    float sphere_radius,
    const float3& sphere_center,
    SqSphereSimplex* simplex,
    float* distance) {
  float3 direction = sq_sub(sphere_center, sq.pos);
  if (sq_len2(direction) < 1e-14f) {
    direction = make_float3(1.0f, 0.0f, 0.0f);
  }

  simplex->count = 0;
  const float3 first = support_superquadric_sphere(
      direction, sq, sphere_radius, sphere_center);
  sq_simplex_push_front(simplex, first);
  direction = sq_scale(first, -1.0f);
  ...
  float prev_dist2 = FLT_MAX;
  const float kRelTol = 1e-6f;
  const float kAbsTol = 1e-8f;

  for (int iter = 0; iter < SQ_SPHERE_GJK_MAX_ITERS; ++iter) {
    const float3 support = support_superquadric_sphere(
        direction, sq, sphere_radius, sphere_center);

    if (sq_dot(support, direction) < 0.0f) {
      *distance = sq_len(direction);
      return false;
    }

    sq_simplex_push_front(simplex, support);

    if (sq_update_simplex_and_direction(simplex, &direction)) {
      *distance = 0.0f;
      return true;
    }

    const float dist2 = sq_len2(direction);
    if (dist2 < 1e-12f) {
      *distance = 0.0f;
      return true;
    }

    const float abs_delta = fabsf(prev_dist2 - dist2);
    if (abs_delta < kAbsTol || abs_delta < kRelTol * prev_dist2) {
      *distance = sq_len(direction);
      return false;
    }
...
__device__ inline static void sq_compute_epa_face(
    SqSphereEPAPolytope* poly,
    int face_idx,
    int a,
    int b,
    int c) {
  SqSphereEPAFace* face = &poly->faces[face_idx];
  face->v[0] = a;
  face->v[1] = b;
  face->v[2] = c;
  face->valid = true;

  const float3 v0 = poly->vertices[a];
  const float3 v1 = poly->vertices[b];
  const float3 v2 = poly->vertices[c];
  float3 normal = sq_cross(sq_sub(v1, v0), sq_sub(v2, v0));
  const float n2 = sq_len2(normal);
  if (n2 < 1e-14f) {
    face->valid = false;
    face->distance = FLT_MAX;
    face->normal = make_float3(0.0f, 0.0f, 0.0f);
    return;
  }

  const float inv_n = rsqrtf(n2);
  normal = sq_scale(normal, inv_n);
  float distance = sq_dot(normal, v0);

  if (distance < 0.0f) {
    const int tmp = face->v[1];
    face->v[1] = face->v[2];
    face->v[2] = tmp;
    normal = sq_scale(normal, -1.0f);
    distance = -distance;
  }
...
__device__ inline static int sq_get_face_slot(SqSphereEPAPolytope* poly) {
  for (int i = 0; i < poly->max_face_index; ++i) {
    if (!poly->faces[i].valid) {
      return i;
    }
  }
  if (poly->max_face_index < SQ_SPHERE_EPA_MAX_FACES) {
    const int slot = poly->max_face_index;
    poly->max_face_index++;
    return slot;
  }
  return -1;
...
__device__ inline static void sq_add_horizon_edge(
    int* edge_a,
    int* edge_b,
    int* edge_count,
    int a,
  ...
  for (int i = 0; i < *edge_count; ++i) {
    if (edge_a[i] == b && edge_b[i] == a) {
      edge_a[i] = edge_a[*edge_count - 1];
      edge_b[i] = edge_b[*edge_count - 1];
      (*edge_count)--;
      return;
    }
  }
...
__device__ inline static float run_epa_superquadric_sphere(
    const Superquadric& sq,
    float sphere_radius,
    const float3& sphere_center,
    const SqSphereSimplex& simplex) {
  if (simplex.count < 4) {
    return 0.0f;
  }
  ...
  SqSphereEPAPolytope poly;
  poly.num_vertices = 4;
  poly.max_face_index = 0;

  for (int i = 0; i < 4; ++i) {
    poly.vertices[i] = simplex.points[i];
  }
  for (int i = 0; i < SQ_SPHERE_EPA_MAX_FACES; ++i) {
    poly.faces[i].valid = false;
  }

  sq_compute_epa_face(&poly, sq_get_face_slot(&poly), 0, 1, 2);
  sq_compute_epa_face(&poly, sq_get_face_slot(&poly), 0, 3, 1);
  sq_compute_epa_face(&poly, sq_get_face_slot(&poly), 0, 2, 3);
  sq_compute_epa_face(&poly, sq_get_face_slot(&poly), 1, 3, 2);

  const float kTolerance = 1e-4f;

  for (int iter = 0; iter < SQ_SPHERE_EPA_MAX_ITERS; ++iter) {
    float best_distance = FLT_MAX;
    int best_face = -1;
    for (int f = 0; f < poly.max_face_index; ++f) {
      if (poly.faces[f].valid && poly.faces[f].distance < best_distance) {
        best_distance = poly.faces[f].distance;
        best_face = f;
      }
    }
    ...
    const float3 direction = poly.faces[best_face].normal;
    const float3 support = support_superquadric_sphere(
        direction, sq, sphere_radius, sphere_center);
    const float support_distance = sq_dot(direction, support);

    if (support_distance - best_distance < kTolerance) {
      return best_distance;
    }

    if (poly.num_vertices >= SQ_SPHERE_EPA_MAX_VERTICES) {
      return best_distance;
    }

    const int new_vertex = poly.num_vertices;
    poly.vertices[new_vertex] = support;
    poly.num_vertices++;

    int edge_a[SQ_SPHERE_EPA_MAX_FACES * 3];
    int edge_b[SQ_SPHERE_EPA_MAX_FACES * 3];
    int edge_count = 0;
    ...
    for (int f = 0; f < poly.max_face_index; ++f) {
      ...
      const SqSphereEPAFace* face = &poly.faces[f];
      const float3 base = poly.vertices[face->v[0]];
      const float3 to_support = sq_sub(support, base);
      if (sq_dot(face->normal, to_support) <= 1e-6f) {
        continue;
      }

      sq_add_horizon_edge(edge_a, edge_b, &edge_count, face->v[0], face->v[1]);
      sq_add_horizon_edge(edge_a, edge_b, &edge_count, face->v[1], face->v[2]);
      sq_add_horizon_edge(edge_a, edge_b, &edge_count, face->v[2], face->v[0]);
      poly.faces[f].valid = false;
    }

    for (int e = 0; e < edge_count; ++e) {
      const int slot = sq_get_face_slot(&poly);
      if (slot < 0) {
        break;
      }
      sq_compute_epa_face(&poly, slot, edge_a[e], edge_b[e], new_vertex);
    }
  }

  float fallback = FLT_MAX;
  for (int f = 0; f < poly.max_face_index; ++f) {
    if (poly.faces[f].valid && poly.faces[f].distance < fallback) {
      fallback = poly.faces[f].distance;
    }
  }

  return (fallback == FLT_MAX) ? 0.0f : fallback;
}

__device__ inline static float fallback_radial_signed_distance_superquadric_sphere(
    const Superquadric& sq,
    float sphere_radius,
    const float3& sphere_center) {
  float3 dir = sq_sub(sphere_center, sq.pos);
  float dir_len = sq_len(dir);
  if (dir_len < 1e-8f) {
    dir = make_float3(1.0f, 0.0f, 0.0f);
    dir_len = 1.0f;
  }

  dir = sq_scale(dir, 1.0f / dir_len);
  const float3 sq_support = support_superquadric_fast(dir, sq);
  const float center_to_surface = sq_dot(dir, sq_sub(sphere_center, sq_support));
  return center_to_surface - sphere_radius;
}

__device__ inline static float evaluate_superquadric_sphere_signed_distance(
    const Superquadric& sq,
    float sphere_radius,
    const float3& sphere_center) {
  const float radius = (sphere_radius < 0.0f) ? -sphere_radius : sphere_radius;

  SqSphereSimplex simplex;
  float gjk_distance = 0.0f;
  const bool colliding = run_gjk_superquadric_sphere(
      sq, radius, sphere_center, &simplex, &gjk_distance);

  if (!colliding) {
    return gjk_distance;
  }

  float penetration = run_epa_superquadric_sphere(sq, radius, sphere_center, simplex);
  if (penetration > 1e-6f) {
    return -penetration;
  }

  // EPA can return ~0 for degenerate low-dimensional simplices.
  // In that case use a radial support-based fallback estimate.
  float fallback_sdf = fallback_radial_signed_distance_superquadric_sphere(
      sq, radius, sphere_center);
  if (fallback_sdf > 0.0f) {
    fallback_sdf = 0.0f;
  }
  return fallback_sdf;
}

__device__ inline static Superquadric superquadric_from_params12(const float* p) {
  Superquadric sq;
  sq.radii = make_float3(p[0], p[1], p[2]);
  sq.eps = make_float2(p[3], p[4]);
  sq.pos = make_float3(p[5], p[6], p[7]);
  sq.quat = make_float4(p[8], p[9], p[10], p[11]);
  return sq;
}

__global__ void compute_superquadric_sphere_signed_distance_kernel(
    Superquadric sq,
    float sphere_radius,
    float3 sphere_center,
  ...
  *signed_distance = evaluate_superquadric_sphere_signed_distance(
      sq, sphere_radius, sphere_center);
}

__global__ void compute_superquadric_sphere_signed_distance_batch_params_kernel(
    const float* sq_params,
    const float* sphere_xyzr,
    int n,
    float* signed_distances) {
  const int idx = static_cast<int>(blockIdx.x * blockDim.x + threadIdx.x);
  if (idx >= n) {
    return;
  }

  const float* sq_p = sq_params + (idx * 12);
  const float* sphere_p = sphere_xyzr + (idx * 4);
  const Superquadric sq = superquadric_from_params12(sq_p);
  const float3 center = make_float3(sphere_p[0], sphere_p[1], sphere_p[2]);
  signed_distances[idx] = evaluate_superquadric_sphere_signed_distance(
      sq, sphere_p[3], center);
}

__global__ void compute_superquadric_sphere_signed_distance_batch_single_sq_kernel(
    const float* sq_param,
    const float* sphere_xyzr,
    int n,
    float* signed_distances) {
  const int idx = static_cast<int>(blockIdx.x * blockDim.x + threadIdx.x);
  ...
  const Superquadric sq = superquadric_from_params12(sq_param);
  const float* sphere_p = sphere_xyzr + (idx * 4);
  const float3 center = make_float3(sphere_p[0], sphere_p[1], sphere_p[2]);
  signed_distances[idx] = evaluate_superquadric_sphere_signed_distance(
      sq, sphere_p[3], center);
...
float compute_superquadric_sphere_signed_distance(
    const Superquadric* sq,
    float sphere_radius,
    float3 sphere_center) {
  if (sq == nullptr) {
  ...
  float* d_signed_distance = nullptr;
  float h_signed_distance = 0.0f;

  cudaMalloc((void**)&d_signed_distance, sizeof(float));
  compute_superquadric_sphere_signed_distance_kernel<<<1, 1>>>(
      *sq, sphere_radius, sphere_center, d_signed_distance);
  cudaDeviceSynchronize();
  cudaMemcpy(&h_signed_distance,
             d_signed_distance,
             sizeof(float),
             cudaMemcpyDeviceToHost);
  cudaFree(d_signed_distance);

  return h_signed_distance;
}

void compute_superquadric_sphere_signed_distance_batch_params(
    const float* sq_params,
    const float* sphere_xyzr,
    int n,
    float* signed_distances,
    cudaStream_t stream) {
  if (n <= 0 || sq_params == nullptr || sphere_xyzr == nullptr || signed_distances == nullptr) {
    return;
  }

  const int threads = 256;
  const int blocks = (n + threads - 1) / threads;
  compute_superquadric_sphere_signed_distance_batch_params_kernel<<<
      blocks,
      threads,
      0,
      stream>>>(sq_params, sphere_xyzr, n, signed_distances);
}

void compute_superquadric_sphere_signed_distance_batch_single_sq(
    const float* sq_param,
    const float* sphere_xyzr,
    int n,
    float* signed_distances,
    cudaStream_t stream) {
  if (n <= 0 || sq_param == nullptr || sphere_xyzr == nullptr || signed_distances == nullptr) {
    return;
  }

  const int threads = 256;
  const int blocks = (n + threads - 1) / threads;
  compute_superquadric_sphere_signed_distance_batch_single_sq_kernel<<<
      blocks,
      threads,
      0,
      stream>>>(sq_param, sphere_xyzr, n, signed_distances);
}